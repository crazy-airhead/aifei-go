...
func TestOutOkFailOf(t *testing.T) {
	if ok := server.Ok(); !ok.IsOk() || ok.Code() != server.CodeOK {
...
func TestOutFile(t *testing.T) {
	o := server.OfFile(func(s *server.FileSender) { s.SetFileName("a.zip") })
...
func TestOutRaw(t *testing.T) {
	...
	r := server.OfRawReader("video/mp4", bytes.NewReader(data))
	if r.RawSize() != 0 {
...
func TestOutHeaders(t *testing.T) {
	o := server.Ok().SetHeaders((&server.Headers{}).SetHeader("X-A", "1"))
	if o.HeadersOut() == nil {
...
func TestOutClear(t *testing.T) {
	o := server.OfRaw("image/png", []byte("x")).SetForward("/f").SetRedirect("/r")
	o.Clear()
...
func TestOutShouldRollback(t *testing.T) {
	if server.Ok().ShouldRollback() {
		t.Fatal("Ok should not rollback")
	}
	if !server.Fail("x").ShouldRollback() {
