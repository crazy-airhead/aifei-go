...
func TestGlobalGetSet(t *testing.T) {
	// Clean up global state after the test (no exported getter for the
	// current global, so reset to nil — equivalent isolation given the
	// package-level global starts nil).
	...
	// Get
	if v := config.Get("name"); v != "test" {
		t.Fatalf("expected 'test', got %v", v)
	}

	// GetStr
	if v := config.GetStr("name"); v != "test" {
	...
	// GetInt64
	if v := config.GetInt64("server.port"); v != 8080 {
		t.Fatalf("expected 8080, got %d", v)
	}

	// GetFloat64
	if v := config.GetFloat64("server.port"); v != 8080.0 {
		t.Fatalf("expected 8080.0, got %f", v)
	}

	// GetBool
	p.Set("enabled", true)
	if v := config.GetBool("enabled"); !v {
		t.Fatal("expected true")
	}
...
func TestGlobalNilProps(t *testing.T) {
	...
	if v := config.Get("key", "default"); v != "default" {
		t.Fatalf("expected 'default', got %v", v)
	}
	if v := config.GetStr("key"); v != "" {
		t.Fatalf("expected empty string, got '%s'", v)
	}
	if v := config.GetStr("key", "fallback"); v != "fallback" {
		t.Fatalf("expected 'fallback', got '%s'", v)
	}
	if v := config.GetBool("key"); v {
		t.Fatal("expected false")
	}
	if v := config.GetBool("key", true); !v {
		t.Fatal("expected default true")
	}
	if v := config.GetInt("key"); v != 0 {
		t.Fatalf("expected 0, got %d", v)
	}
	if v := config.GetInt("key", 42); v != 42 {
		t.Fatalf("expected 42, got %d", v)
	}
	if v := config.GetInt64("key"); v != 0 {
		t.Fatalf("expected 0, got %d", v)
	}
	if v := config.GetFloat64("key"); v != 0 {
		t.Fatalf("expected 0, got %f", v)
	}
	if config.Has("key") {
		t.Fatal("expected false")
	...
	var db DBConf
	if err := config.SubBind("db", &db); err != nil {
...
func TestGlobalGetDefaults(t *testing.T) {
	...
	// Get with default
	if v := config.Get("missing", "default"); v != "default" {
		t.Fatalf("expected 'default', got %v", v)
	}

	// GetStr with default
	if v := config.GetStr("missing", "fallback"); v != "fallback" {
		t.Fatalf("expected 'fallback', got '%s'", v)
	}
	// empty string triggers default
	p.Set("empty", "")
	if v := config.GetStr("empty", "nonempty"); v != "nonempty" {
		t.Fatalf("expected 'nonempty', got '%s'", v)
	}

	// GetBool with default
	if v := config.GetBool("missing", true); !v {
		t.Fatal("expected default true")
	}
	// non-bool triggers default
	p.Set("name", "hello")
	if v := config.GetBool("name", true); !v {
		t.Fatal("expected default true for non-bool")
	}

	// GetInt with default
	if v := config.GetInt("missing", 3000); v != 3000 {
		t.Fatalf("expected 3000, got %d", v)
	}

	// GetInt64 with default
	if v := config.GetInt64("missing", 100); v != 100 {
		t.Fatalf("expected 100, got %d", v)
	}

	// GetFloat64 with default
	if v := config.GetFloat64("missing", 3.14); v != 3.14 {
		t.Fatalf("expected 3.14, got %f", v)
	}
}
...
func TestSetProps(t *testing.T) {
	...
	if v := config.GetStr("key"); v != "value" {
		t.Fatalf("expected 'value', got '%s'", v)
	}

	// SetProps with nil
	config.SetProps(nil)
	// Keys() returns nil when the global is nil.
	if config.Keys() != nil {
		t.Fatal("expected nil keys (nil global) after SetProps(nil)")
	}
}

// =============================================================================
// Key normalization tests (black-box via exported Set/Get/Has)
// =============================================================================
