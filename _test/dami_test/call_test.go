...
func TestCallRequestResponse(t *testing.T) {
	b := dami.New()
	dami.ListenCallOn(b, "demo.hello", func(data string) (string, error) {
		return "hi:" + data, nil
	})
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := dami.CallOn[string, string](b, "demo.hello", "world").Get(ctx)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if r != "hi:world" {
...
func TestCallError(t *testing.T) {
	b := dami.New()
	dami.ListenCallOn(b, "op", func(data int) (int, error) {
		return 0, errors.New("boom")
	})
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := dami.CallOn[int, int](b, "op", 1).Get(ctx)
...
func TestCallFallback(t *testing.T) {
	...
	fut := dami.CallOn[string, string](b, "nope", "x", func(f *dami.Future[string]) {
...
func TestCallCompute(t *testing.T) {
	b := dami.New()
	dami.ListenCallOn(b, "op", func(data int) (int, error) { return data * 2, nil })
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := dami.CallOn[int, int](b, "op", 21).Get(ctx)
	if err != nil || r != 42 {
...
func TestCallTimeout(t *testing.T) {
	...
	defer cancel()
	r, err := dami.CallOn[int, int](b, "nobody", 1).Get(ctx)
...
func TestCallDefaultBus(t *testing.T) {
	...
	un := dami.ListenCall(topic, func(data int) (int, error) { return data + 1, nil })
	defer un()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := dami.Call[int, int](topic, 10).Get(ctx)
	if err != nil || r != 11 {
		t.Fatalf("r=%d err=%v", r, err)
	}
}
