...
func TestStreamItems(t *testing.T) {
	b := dami.New()
	dami.ListenStreamOn(b, "gen", func(data string, sink dami.StreamSink[int]) {
		for i := 1; i <= 3; i++ {
			sink.Next(i)
		}
		sink.Complete(nil)
	...
	defer cancel()
	var got []int
	for it := range dami.StreamOn[string, int](b, ctx, "gen", "go") {
		if it.Err != nil {
			t.Fatal(it.Err)
		}
...
func TestStreamCancel(t *testing.T) {
	...
	defer cancel()
	var n int
	for range dami.StreamOn[string, int](b, ctx, "gen", "go") {
		n++
	}
	if n != 2 {
...
func TestStreamDefaultBus(t *testing.T) {
	...
	un := dami.ListenStream(topic, func(data string, sink dami.StreamSink[string]) {
		sink.Next(data + "!")
		sink.Complete(nil)
	})
	defer un()
	...
	defer cancel()
	var got []string
	for it := range dami.Stream[string, string](ctx, topic, "hi") {
		if it.Err != nil {
			t.Fatal(it.Err)
		}
		got = append(got, it.Val)
