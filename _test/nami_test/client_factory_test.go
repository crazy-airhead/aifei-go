...
func TestClientFactoryServiceNameInherited(t *testing.T) {
	...
	f1 := nami.NewClientFactory().Name("svc-a")
	f2 := nami.NewClientFactory().ServiceName("svc-b")
...
func TestClientFactoryMultipleFilters(t *testing.T) {
	f := nami.NewClientFactory().
		FilterAdd(nami.FilterFunc(func(inv *nami.Invocation) (*nami.Result, error) { return inv.Invoke() })).
		FilterAdd(nami.FilterFunc(func(inv *nami.Invocation) (*nami.Result, error) { return inv.Invoke() }))
	...
	c.Config().FilterAdd(nami.FilterFunc(func(inv *nami.Invocation) (*nami.Result, error) { return inv.Invoke() }))
