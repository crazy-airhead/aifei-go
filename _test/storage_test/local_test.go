...
func TestLocalRoundTrip(t *testing.T) {
	c, err := storage.NewLocalClient("bucket", t.TempDir(), nil)
	if err != nil {
		t.Fatalf("NewLocalClient: %v", err)
	}

	// Put with a nested key.
	pr, err := c.Put("a/b.txt", storage.OfString("hello world", "text/plain"))
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
	...
	// TempURL unsupported.
	if _, err := c.TempURL("a/b.txt", time.Minute); !errors.Is(err, storage.ErrUnsupported) {
		t.Fatalf("TempURL err = %v, want ErrUnsupported", err)
	}
}

func TestLocalDeleteAndBatch(t *testing.T) {
	c, _ := storage.NewLocalClient("bucket", t.TempDir(), nil)
	if _, err := c.Put("x.txt", storage.OfString("1", "")); err != nil {
		t.Fatal(err)
	}
	if _, err := c.Put("y.txt", storage.OfString("2", "")); err != nil {
		t.Fatal(err)
	}
...
func TestLocalPutResultParentAbsolute(t *testing.T) {
	root := t.TempDir()
	c, _ := storage.NewLocalClient("bucket", root, nil)
	pr, err := c.Put("c.txt", storage.OfString("x", ""))
	if err != nil {
		t.Fatal(err)
	}
