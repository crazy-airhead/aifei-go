...
func TestOfBytes(t *testing.T) {
	m := storage.OfBytes([]byte("hello"), "text/plain")
...
func TestOfString(t *testing.T) {
	m := storage.OfString("world", "text/plain")
	s, err := m.String()
	if err != nil {
...
func TestOfFileNameMimeInference(t *testing.T) {
	...
	for name, want := range cases {
		m := storage.OfFileName(name, strings.NewReader(""))
	...
	m := storage.OfFileName("blob.unknownext", strings.NewReader(""))
	if m.ContentType() != "text/plain; charset=utf-8" {
		t.Fatalf("unknown ext: ContentType = %q, want %q", m.ContentType(), "text/plain; charset=utf-8")
	}
}
...
func TestMediaClose(t *testing.T) {
	...
	m2 := storage.NewMedia(strings.NewReader("y"), "")
