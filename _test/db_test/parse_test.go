...
func TestParseFromSingleTable(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "user"}})
	assertProjections(t, r, []dbsql.Projection{{Star: true, Column: "*"}})
}

func TestParseFromWithAlias(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "u"}})
	assertAliasToTable(t, r, map[string]string{"user": "user", "u": "user"})
}

func TestParseFromWithAS(t *testing.T) {
	r := dbsql.Parse("SELECT u.name FROM user AS u")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "u"}})
	assertProjections(t, r, []dbsql.Projection{{TableAlias: "u", Column: "name", Label: "name"}})
}

func TestParseFromMultipleTablesComma(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u, dept d")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
}
...
func TestParseJoinInner(t *testing.T) {
	r := dbsql.Parse("SELECT u.name, d.name AS dept_name FROM user u INNER JOIN dept d ON u.dept_id = d.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
	assertProjections(t, r, []dbsql.Projection{
...
func TestParseJoinLeft(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u LEFT JOIN dept d ON u.dept_id = d.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func TestParseJoinRight(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u RIGHT JOIN dept d ON u.dept_id = d.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func TestParseJoinFullOuter(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u FULL OUTER JOIN dept d ON u.dept_id = d.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func TestParseJoinCross(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user CROSS JOIN dept")
	assertTables(t, r, []dbsql.TableRef{
...
func TestParseJoinNatural(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user NATURAL JOIN dept")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "user"},
		{Table: "dept", Alias: "dept"},
	})
...
func TestParseJoinBare(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u JOIN dept d ON u.dept_id = d.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func TestParseJoinUsing(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u JOIN dept d USING (dept_id)")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func TestParseMultipleJoins(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u JOIN dept d ON u.dept_id = d.id JOIN role r ON d.role_id = r.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
		{Table: "role", Alias: "r"},
...
func TestParseSelectAliasStar(t *testing.T) {
	r := dbsql.Parse("SELECT u.*, d.name FROM user u JOIN dept d ON u.dept_id = d.id")
	assertProjections(t, r, []dbsql.Projection{
...
func TestParseSelectBareColumn(t *testing.T) {
	r := dbsql.Parse("SELECT name, age FROM user")
	assertProjections(t, r, []dbsql.Projection{
...
func TestParseSelectQualifiedColumn(t *testing.T) {
	r := dbsql.Parse("SELECT u.name, u.age FROM user u")
	assertProjections(t, r, []dbsql.Projection{
		{TableAlias: "u", Column: "name", Label: "name"},
...
func TestParseSelectColumnAsLabel(t *testing.T) {
	r := dbsql.Parse("SELECT name AS n, age AS a FROM user")
	assertProjections(t, r, []dbsql.Projection{
		{Column: "name", Label: "n"},
		{Column: "age", Label: "a"},
	})
...
func TestParseSelectQualifiedColumnAsLabel(t *testing.T) {
	r := dbsql.Parse("SELECT u.name AS user_name FROM user u")
	assertProjections(t, r, []dbsql.Projection{
...
func TestParseSelectFunctionAlias(t *testing.T) {
	...
	r := dbsql.Parse("SELECT COUNT(*) AS cnt FROM user")
	assertProjections(t, r, []dbsql.Projection{
		{Label: "cnt"},
	})
}
...
func TestParseSelectFunctionNoAlias(t *testing.T) {
	...
	r := dbsql.Parse("SELECT COUNT(*) FROM user")
...
func TestParseSubqueryDerivedTable(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM (SELECT id, name FROM user) AS sub")
	assertTables(t, r, []dbsql.TableRef{
...
func TestParseSubqueryDerivedTableNoAs(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM (SELECT id FROM user) sub")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "sub", Alias: "sub", FromSubquery: true},
	})
}
...
func TestParseSchemaPrefix(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM db.user AS u")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "u"}})
}
...
func TestParseMixedCase(t *testing.T) {
	r := dbsql.Parse("Select u.Name, d.Name As dept_name From user u Join dept d On u.dept_id = d.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func TestParseUnionFirstBranch(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u UNION SELECT * FROM admin a")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "u"}})
}

func TestParseSelectNoFrom(t *testing.T) {
	r := dbsql.Parse("SELECT 1")
	assertTables(t, r, nil)
	assertProjections(t, r, nil)
}
...
func TestParseWhereStopsFrom(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user WHERE age > 10")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "user"}})
}

func TestParseGroupByStopsFrom(t *testing.T) {
	r := dbsql.Parse("SELECT dept_id, COUNT(*) AS cnt FROM user GROUP BY dept_id")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "user"}})
}

func TestParseOrderByStopsFrom(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user ORDER BY age")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "user"}})
}

func TestParseLimitStopsFrom(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user LIMIT 10")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "user"}})
}

func TestParseLeftOuterJoin(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u LEFT OUTER JOIN dept d ON u.dept_id = d.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func TestParseExpressionAs(t *testing.T) {
	...
	r := dbsql.Parse("SELECT 1 + 1 AS result FROM user")
	assertProjections(t, r, []dbsql.Projection{{Label: "result"}})
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "user"}})
}

func TestParseWithStringLiterals(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user WHERE name = 'hello'")
	assertTables(t, r, []dbsql.TableRef{{Table: "user", Alias: "user"}})
}

func TestParseNestedParens(t *testing.T) {
	r := dbsql.Parse("SELECT * FROM user u JOIN dept d ON (u.a = d.a AND (u.b = d.b OR u.c = d.c))")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func TestParseNeverNil(t *testing.T) {
	...
	if r.Tables == nil {
		t.Fatal("dbsql.Result.Tables must never be nil")
	}
	if r.AliasToTable == nil {
		t.Fatal("dbsql.Result.AliasToTable must never be nil")
	}
	if r.Projections == nil {
		t.Fatal("dbsql.Result.Projections must never be nil")
	}
}
...
func TestProjectionEquality(t *testing.T) {
	a := dbsql.Projection{TableAlias: "u", Column: "name", Label: "name"}
	b := dbsql.Projection{TableAlias: "u", Column: "name", Label: "name"}
...
func TestRealWorldJoin(t *testing.T) {
	r := dbsql.Parse("SELECT u.*, d.name AS dept_name, d.config AS dept_config FROM user u JOIN dept d ON u.dept_id = d.id")
	assertTables(t, r, []dbsql.TableRef{
		{Table: "user", Alias: "u"},
		{Table: "dept", Alias: "d"},
	})
...
func assertTables(t *testing.T, r *dbsql.Result, expected []dbsql.TableRef) {
	t.Helper()
...
func assertProjections(t *testing.T, r *dbsql.Result, expected []dbsql.Projection) {
	t.Helper()
...
func assertAliasToTable(t *testing.T, r *dbsql.Result, expected map[string]string) {
	t.Helper()
