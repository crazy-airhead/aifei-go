...
| `GetStr/GetInt/GetInt64/GetFloat64/GetBool/GetTime/GetBytes` | 已转型，宽松语义（调用 `type_converter.go` 的 `ToString/ToInt/...`；NULL/缺失/脏数据→零值） |
| `GetTimeE(field) (time.Time, error)` | 严格版 GetTime：解析失败返回 error（NULL 返回零值 + nil） |
...
`ToInt` / `ToInt64` / `ToFloat64` / `ToBool` / `ToString` / `ToTime` 都是 nil 安全、多源适配的**宽松**转换函数（失败回零值），`Row.GetStr/GetInt/...` 与 `Kv.GetStr/...` 都基于它们。`ToTimeE` 是严格版：依次尝试 `"2006-01-02 15:04:05"`、RFC3339(Nano)、`"2006-01-02"`、`"15:04:05"` 等 layout，**解析失败返回明确 error**；`nil`（列 NULL）返回 `(零值, nil)`——NULL 是缺值不是脏数据。`Row.GetTimeE(field)` 同签名。

**时间列在 scan 时即类型化**：查询结果中声明为 DATE/TIME/DATETIME/TIMESTAMP 的列会直接解析成 `time.Time` 存入 Row（无论驱动返回 string 还是 `[]byte`），脏数据会在查询处报错（错误信息含列名）——所以宽松的 `GetTime` 拿到的查询结果都是已验证的值，零值兜底实际上只可能出现在手工构建的 Row 或 JSON 输入上。JSON 序列化输出按 `TimeFormat`（`2006-01-02 15:04:05`）格式化，与旧行为一致。MySQL 用户建议 DSN 加 `parseTime=true`（驱动原生返回 `time.Time`）。
