...
func TestHandleJSON(t *testing.T) {
	...
	h.Handle(rec, nil, Of(map[string]interface{}{"k": "v"}))

	if ct := rec.Header().Get("Content-Type"); !strings.HasPrefix(ct, "application/json") {
		t.Fatalf("content-type: %q", ct)
	}
	m := decodeBody(t, rec)
	if m["code"] != float64(CodeOK) {
...
func TestHandleNilAndPlainOutput(t *testing.T) {
	...
	if decodeBody(t, rec)["code"] != float64(CodeOK) {
...
func TestHandleRedirect(t *testing.T) {
	h := NewIoHandler(nil)
	rec := httptest.NewRecorder()
	h.Handle(rec, nil, Redirect("/elsewhere", http.StatusMovedPermanently))
...
func TestHandleHeaders(t *testing.T) {
	hdr := (&Headers{}).
		...
		AddCookie(Cookie{Name: "c", Value: "v"})
	h := NewIoHandler(nil)
	rec := httptest.NewRecorder()
	h.Handle(rec, nil, Ok().SetHeaders(hdr))
...
func TestHandleRaw(t *testing.T) {
	...
	h.Handle(rec, nil, OfRaw("image/png", body))
...
func TestHandleRawReaderSize(t *testing.T) {
	h := NewIoHandler(nil)
	rec := httptest.NewRecorder()
	h.Handle(rec, nil, OfRawReader("application/pdf", strings.NewReader("pdf-bytes")).SetRawSize(9))
...
func TestHandleFileData(t *testing.T) {
	h := NewIoHandler(nil)
	rec := httptest.NewRecorder()
	h.Handle(rec, nil, OfFile(func(s *FileSender) {
...
func TestHandleFileDisk(t *testing.T) {
	...
	h := NewIoHandler(nil, WithDownloadBase(dir))
	rec := httptest.NewRecorder()
	h.Handle(rec, nil, OfFile(func(s *FileSender) { s.SetFileName("f.txt") }))
...
func TestHandleFileMissingSourceFallsBackToJSON(t *testing.T) {
	h := NewIoHandler(nil)
	rec := httptest.NewRecorder()
	h.Handle(rec, nil, OfFile(func(s *FileSender) { /* no source set */ }))

	if !strings.HasPrefix(rec.Header().Get("Content-Type"), "application/json") {
		t.Fatalf("expected JSON fallback, content-type: %q", rec.Header().Get("Content-Type"))
	}
	if m := decodeBody(t, rec); m["code"] != float64(CodeFail) {
...
func TestHandleView(t *testing.T) {
	...
	h := NewIoHandler(nil, WithBaseTemplatePath(dir))
	rec := httptest.NewRecorder()
	h.Handle(rec, nil, Of("ignored").SetView("t.html").SetData(map[string]interface{}{}).Set("msg", "world"))
...
func TestServeHTTPForwardAndNotFound(t *testing.T) {
	app := aifei.New()
	app.Router().Handle("GET", "/json", func(in aifei.Input) aifei.Output { return Of("ok") })
	app.Router().Handle("GET", "/a", func(in aifei.Input) aifei.Output { return Forward("/b") })
	app.Router().Handle("GET", "/b", func(in aifei.Input) aifei.Output { return Of("done") })
	app.Router().Handle("GET", "/loop", func(in aifei.Input) aifei.Output { return Forward("/loop") })
	...
	if m := decodeBody(t, rec3); m["code"] != float64(CodeFail) {
